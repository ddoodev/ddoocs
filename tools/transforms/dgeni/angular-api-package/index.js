...
module.exports = new Package('angular-api', [basePackage, typeScriptPackage])
  ...
  .processor(require('./processors/normalizePaths').normalizePaths)
  .processor(require('./processors/computeFunctionReturnType').computeFunctionReturnType)
  ...
  .config(function(readTypeScriptModules, readFilesProcessor, tsParser) {
    ...
    // Tell TypeScript how to load modules that start with `@src`
    tsParser.options.paths = { '@src/*': [API_SOURCE_PATH + '/*'] };
  ...
  .config(function(parseTagsProcessor) {
    ...
    typeTags.forEach(typeTag => typeTag.transforms[0] = function dummyTypeTransform(doc, tag, value) {
      const TYPE_EXPRESSION_START = /^\s*\{[^@]/;
      let start, position, count, length;
  ...
  .config(function(computePathsProcessor, EXPORT_DOC_TYPES, generateApiListDoc, generateDeprecationListDoc) {
    ...
    computePathsProcessor.pathTemplates.push({
      ...
      getPath: function computeModulePath(doc) {
        doc.moduleFolder = `${API_SEGMENT}/${doc.id.replace('/src', '').replace(/\/index$/, '')}`;
    ...
    computePathsProcessor.pathTemplates.push({
      ...
      getPath: (doc) => {
        return `${API_SEGMENT}/${doc.id.replace('/src', '').replace(/^index\//, `index/${doc.docType}/`)}`;
      },
